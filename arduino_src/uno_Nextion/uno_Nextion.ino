...
void loop() {
  unsigned long nowtime = millis(); // 루프문 동작이 0.4초 이상 지났을 때마다 데이터 송신 
  start_time = millis();
  ...
  if (SerialForNex.available() > 0) {   // Nextion LCD 데이터 수신 확인 
  ...
  if (sum_time > 400) { // 0.4초 이상 지났을 때 데이터 보냄
    // 결과 출력 및 Nextion으로 데이터 전송
    SerialForNex.print(F("tTemp.txt="));
    SerialForNex.print("\"");
...
void readPMS() { // RTC, Sensor 보드에서 parsing 받아오는 함수
  // Data Parsing
  char input;
  if (Serial.available()) {
    ...
    if (count == 3) {
      ...
      // 미세먼지 농도 확인 후 그에 맞는 이미지 디스플레이에 출력
      if (PM10 >= 80 || PM25 >= 35) {
